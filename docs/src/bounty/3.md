...
Doginal bounty 3 has two parts, both of which are based on *ordinal names*.
Doginal names are a modified base-26 encoding of ordinal numbers. To avoid
...
To search an `dog` wallet for sats with a name in `frequency.tsv`, use the
following [`dog`](https://github.com/jonheaven/dog) command:

```
dog wallet sats --tsv frequency.tsv
```

This command requires the sat index, so `--index-koinu` must be passed to `dog`
